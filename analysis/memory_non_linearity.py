...
def sort_data(data):
    ...
    new_data = []
    for d in data:
        if float(d['force_regularization']) == REG:
...
def plot_nl_mem(data):
    ...
    plt.ylabel('ELM Size')
    plt.axis([MIN_DL, MAX_DL, MIN_ELM, MAX_ELM])
    ...
    plt.plot(x, y, 'k.')
    plt.colorbar()
    plt.title("PERFORMANCE: Poower Consumption")
    plt.xlabel('Delay Line Size')
    plt.ylabel('ELM Size')
    plt.axis([MIN_DL, MAX_DL, MIN_ELM, MAX_ELM])
