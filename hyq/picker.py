...
class Picker:
    ...
    def getIndex(self):
        ...
        indexes = []
        for i, a in enumerate(self.all_options):
            if a["selected"] == True:
                indexes.append(i)
    ...
    def redraw(self):
        ...
        for option in range:
            if not self.numeral:
                ...
                line_label = "[" + str(option["selected"]) + "] "
    ...
    def curses_loop(self, stdscr):
        while 1:
            ...
            elif c in [ord('0'), ord('1'), ord('2'), ord('3'), ord('4'), 
                       ord('5'), ord('6'), ord('7'), ord('8'), ord('9')]:
                self.all_options[self.selected]["selected"] = chr(c)
            elif c == 10:
                break
    ...
    def __init__(
        ...
        if not self.numeral:
            self.init_options = [False for _ in options]
        else:
            self.init_options = [0 for _ in options]

        if init_options is not None:
            if len(init_options) == len(options):
